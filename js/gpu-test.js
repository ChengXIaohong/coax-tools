// 提示用户可能造成卡顿，确认后执行
if (!confirm("警告：此测试可能会导致设备卡顿或发热，尤其是在性能较弱的设备上。\n\n点击确定继续，点击取消退出。")) {
    // 用户取消，不执行测试
    throw new Error("用户取消了测试");
}
var cx, cy;
var glposition;
var glright;
...
function resize() {
    cx = document.getElementById("main").clientWidth;
    cy = document.getElementById("main").clientHeight;
    if(cx>cy){
        cx=cy;
    }
    else{
    ...
    // 移除之前的缩放变换，直接设置canvas大小
    document.getElementById("main").style.transform = "none";
    document.getElementById("c1").width = cx;
    document.getElementById("c1").height = cy;
}
